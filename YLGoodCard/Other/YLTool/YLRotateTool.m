...
+ (void)bannerWithParam:(id)param success:(void (^)(NSArray<YLBannerModel *> *result))success failure:(void (^)(NSError * error))failure {
    ...
    [self getWithUrl:url param:param arrayForResultClass:[YLBannerModel class] success:success failure:failure];
}
...
+ (void)notableWithParam:(id)param success:(void (^)(NSArray<YLNotableModel *> *result))success failure:(void (^)(NSError * error))failure {
    ...
    [self getWithUrl:url param:param arrayForResultClass:[YLNotableModel class] success:success failure:failure];
//    NSLog(@"%@",success);
}
...
+ (void)recommendWithParam:(id)param success:(void (^)(NSArray<YLTableViewModel *> *result))success failure:(void (^)(NSError * _Nonnull))failure {
    ...
    [self getWithUrl:url param:param arrayForResultClass:[YLTableViewModel class] success:success failure:failure];
}

+ (void)buyWithParam:(id)param success:(void (^)(NSArray<YLTableViewModel *> * _Nonnull))success failure:(void (^)(NSError * _Nonnull))failure {
    
//    NSString *url = @"http://ucarjava.bceapp.com/detail?method=search";
    NSString *url = @"http://ucarjava.bceapp.com/detail?method=search";
    [self getWithUrl:url param:param arrayForResultClass:[YLTableViewModel class] success:success failure:failure];
}
